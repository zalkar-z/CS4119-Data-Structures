...
#ifndef TASK_1_DOUBLYLINKEDLIST_H
#define TASK_1_DOUBLYLINKEDLIST_H
...
public:
     ...
     *            and simultaneously compare it with stack and
     *            return specific error codes if needed
     *
     * The function is defensive for empty linked lists
     *
     * Returns: 1 - If no errors occured
     *          0 - If the stack and doubly linked list don't match
     *          -1 - If the stack length is bigger than doubly linked list
     */
    int traverse() {
        // hey let's make a stack of ints, how exciting!
        ...
        while(current != NULL) {
            ...
            // stores the last node, which is not NULL
            if (current != NULL) lastNode = current;
        }

        // since current is NULL now, we get the last non-NULL node
        current = lastNode;

        while(current != NULL) {
            if (current->value != s->pop()) return 0;
            current = current->prev;
        }
        // return error if stack length doesn't match with linked list's
        if (!s->empty()) return -1;
        // no errors
        return 1;
    }
...
#endif //TASK_1_DOUBLYLINKEDLIST_H
